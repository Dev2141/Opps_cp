...
function mountTemplates() {
  const encodeTemplate = document.getElementById("encodeTemplate");
  const decodeTemplate = document.getElementById("decodeTemplate");
  const shareTemplate = document.getElementById("shareTemplate");
  const analyzeTemplate = document.getElementById("analyzeTemplate");

  document.getElementById("encodeCard").appendChild(encodeTemplate.content.cloneNode(true));
  document.getElementById("decodeCard").appendChild(decodeTemplate.content.cloneNode(true));
  document.getElementById("shareCard").appendChild(shareTemplate.content.cloneNode(true));
  document.getElementById("analyzeCard").appendChild(analyzeTemplate.content.cloneNode(true));

  // Avoid duplicate IDs in older DOM engines by removing templates post-mount.
  encodeTemplate.remove();
  decodeTemplate.remove();
  shareTemplate.remove();
  analyzeTemplate.remove();
}

function setStatus(message, kind = "info") {
  const dot = document.getElementById("statusDot");
  const text = document.getElementById("statusText");
  const map = { info: "#1462ff", success: "#188958", warn: "#b36c07", error: "#bf2f2f" };
  const color = map[kind] || map.info;
  dot.style.background = color;
  dot.style.boxShadow = `0 0 0 4px ${hexToRgba(color, 0.14)}`;
...
function buildEnvelope(type, fileName, mimeType, dataBytes) {
  ...
  // 64-bit length field; currently using 32-bit payload lengths for compatibility.
  out[p++] = 0;
  out[p++] = 0;
  out[p++] = 0;
  out[p++] = 0;
...
function lsbEmbed(imageData, payload) {
  ...
  for (let i = 0; i < data.length && bitIdx < totalBits; i += 4) {
    ...
    // Keep pixels fully opaque to avoid alpha/premultiply artifacts corrupting LSB data.
    data[i + 3] = 255;
  }
}
...
function lsbExtract(imageData) {
  ...
  if (len <= 0 || len > raw.length - 4) {
    throw new Error("No hidden data found (make sure you selected the stego PNG; JPEG will not work).");
  }
...
async function handleEmbed() {
  try {
    if (!state.carrierImage) return setStatus("Upload a carrier image first.", "warn");
    const pw = document.getElementById("encodePassword").value;
    if (!pw) return setStatus("Encryption password is required.", "warn");
    ...
    document.getElementById("saveStegoBtn").disabled = false;

    // Reduce user error: also load the freshly generated stego into the Decode panel.
    try {
      ...
      // Ignore; embed already succeeded.
    }

    setStatus("Embedding complete. Stego image is ready.", "success");
  } catch (err) {
...
function setDecodeResult(text, kind = "info") {
  ...
  el.textContent = text;
  el.style.color = kind === "error" ? "#9f2626" : kind === "warn" ? "#8f5500" : "#1f3b65";
}
...
async function handleExtract() {
  try {
    ...
    if (looksLikeEnvelope(plain)) {
      ...
      setDecodeResult(textDecoder.decode(plain), "success"); // legacy payload support
    }
    setStatus("Decode successful.", "success");
  } catch (err) {
...
function renderInbox(messages) {
  ...
  body.innerHTML = "";
  if (!messages.length) {
    body.innerHTML = '<tr><td colspan="4">No messages in mailbox</td></tr>';
    return;
  }
  for (const m of messages) {
    const tr = document.createElement("tr");
    if (m.messageId === state.selectedMessageId) tr.classList.add("active");
    tr.innerHTML = `
      <td>${escapeHtml(m.messageId.slice(0, 8))}...</td>
      <td>${escapeHtml(m.sender || "unknown")}</td>
      <td>${escapeHtml(formatTime(m.timestamp))}</td>
...
async function handleRefreshInbox() {
  try {
    ...
    if (state.selectedMessageId && !state.inboxMessages.find(x => x.messageId === state.selectedMessageId)) state.selectedMessageId = "";
    renderInbox(state.inboxMessages);
...
async function handleDownloadSelected() {
  try {
    if (!state.selectedMessageId) return setStatus("Select an inbox message first.", "warn");
    const creds = getMailboxCreds();
    if (!creds.mailboxId || !creds.passphrase) return setStatus("Mailbox ID and passphrase are required.", "warn");
    ...
    setStatus(`Downloaded and loaded ${res.messageId} for decode.`, "success");
  } catch (err) {
...
function handleAnalyze() {
  try {
    ...
    document.getElementById("metricTransition").textContent =
      `${r.transition.toFixed(4)} | hdr=${r.headerLength} (${r.headerPlausible ? "ok" : "no"})`;
...
async function loadCarrier(file) {
  ...
  state.stegoDataUrl = "";
  document.getElementById("saveStegoBtn").disabled = true;
...
function bindEvents() {
  ...
  document.getElementById("secretText").addEventListener("input", updateCapacityStats);
  ...
  carrierDrop.addEventListener("dragover", (e) => e.preventDefault());
  ...
  document.getElementById("decodeImageFile").addEventListener("change", async (e) => {
    ...
    if (!isLosslessStegoDecodeType(f)) {
      ...
      setDecodeResult("This file type is not supported for decode. Use the stego PNG output (JPEG is lossy and destroys LSB data).", "warn");
      setStatus("Decode requires a PNG/BMP stego image.", "warn");
      return;
    }
